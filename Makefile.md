...
​	静态链接库：名字一般libxxx.a，利用静态库编译成的文件一般比较大，因为整个函数库的所有数据都会被整合进目标代码，他的优点就显而易见了，即编译后的执行程序不需要外部的函数库支持，也就是不用部署了，运行时不用加载库文件，运行速度快，因为所有使用的函数都已经被编译进去了。当然这也会成为他的缺点，因为如果静态函数库改变了，那么你的程序必须重新编译。
